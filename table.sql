...
CREATE TABLE users(	--ユーザーテーブル
	...
	user_types_id	integer		NOT NULL references usertypes(id) ON DELETE RESTRICT, --ユーザー種別[個人、法人]
...
CREATE TABLE tells(	--電話番号テーブル
	id		serial		NOT NULL PRIMARY KEY,
	users_id 	uuid		references users(id) NOT NULL ON DELETE CASCADE, --ユーザーID
	tell_stypes_id 	integer		references telltypes(id) NOT NULL ON DELETE RESTRICT, --電話番号種別
...
CREATE TABLE emails(	--メールテーブル
	id		serial		NOT NULL PRIMARY KEY,
	users_id 	uuid		references users(id) NOT NULL ON DELETE CASCADE, --ユーザーID
	email_types_id 	integer		references emailtypes(id) NOT NULL ON DELETE RESTRICT, --メール種別
...
CREATE TABLE addrs(	--住所テーブル
	id		serial		NOT NULL PRIMARY KEY,
	users_id 	uuid		references users(id) NOT NULL ON DELETE CASCADE, --ユーザーID
	addr_types_id 	integer		references addr_types(id) NOT NULL ON DELETE RESTRICT --住所種別
...
CREATE TABLE urls( -- urlテーブル
	id		serial		NOT NULL PRIMARY KEY,
	users_id 	uuid		references users(id) NOT NULL ON DELETE CASCADE, --ユーザーID
	url_types_id 	integer		references url_types(id) NOT NULL ON DELETE RESTRICT, --url種別
	url 		TEXT, 		--url
	memo 		TEXT,		--メモ
);
...
CREATE TABLE users_tags(	--ユーザー タグの中間テーブル
	id		serial		NOT NULL PRIMARY KEY,
	tags_id		integer		references tags(id) NOT NULL ON DELETE CASCADE, --タグID
	users_id 	uuid		references users(id) NOT NULL ON DELETE CASCADE, --ユーザーID
);
